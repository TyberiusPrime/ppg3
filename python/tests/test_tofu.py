...
- Unit tests on the libcst-backed patcher (``tofu._patch_file``) against
  ...
  ``_FakeJob`` stand-in that only needs ``.publish``/``.url``/``._call_site``.
...
# and `tofu.run_tofu_pass` only ever touch `.publish` (the publish map),
# `.url`, and `._call_site` on a job — never `.job_def()`/`.blake3`/anything
# that would need `graph.default_python` or hashing.
class _FakeJob:
    def __init__(self, publish_path, url, call_site):
        self.publish = {"file": publish_path}
        self.url = url
        self._call_site = call_site
        self.id = publish_path


_DIGEST = "ab" * 32


def _patch(tmp_path, source, lineno, outputs="x", url="y", digest=_DIGEST):
    path = tmp_path / "pipeline.py"
    path.write_text(source)
    job = _FakeJob(outputs, url, (str(path), lineno))
...
def test_patch_inserts_missing_kwarg_exact_text(tmp_path, capsys):
    src = 'import ppg3\njob = ppg3.FetchJob(outputs="x", url="y")\n'
    out, unresolved = _patch(tmp_path, src, 2)
    assert unresolved == []
    assert out == f'import ppg3\njob = ppg3.FetchJob(outputs="x", url="y", blake3="{_DIGEST}")\n'
...
def test_patch_replaces_none_value(tmp_path):
    src = 'job = ppg3.FetchJob(outputs="x", url="y", blake3=None)\n'
    out, unresolved = _patch(tmp_path, src, 1)
    assert unresolved == []
    assert out == f'job = ppg3.FetchJob(outputs="x", url="y", blake3="{_DIGEST}")\n'


def test_patch_replaces_none_value_regardless_of_arg_order(tmp_path):
    src = 'job = ppg3.FetchJob(blake3=None, outputs="x", url="y")\n'
    out, unresolved = _patch(tmp_path, src, 1)
    assert unresolved == []
    assert out == f'job = ppg3.FetchJob(blake3="{_DIGEST}", outputs="x", url="y")\n'
...
def test_patch_aliased_module_import(tmp_path):
    ...
    src = 'import ppg3 as p\njob = p.FetchJob(outputs="x", url="y")\n'
    out, unresolved = _patch(tmp_path, src, 2)
    assert unresolved == []
    assert out == (
        'import ppg3 as p\n'
        f'job = p.FetchJob(outputs="x", url="y", blake3="{_DIGEST}")\n'
    )


def test_patch_two_calls_one_line_falls_back(tmp_path, capsys):
    src = 'a = ppg3.FetchJob(outputs="x", url="y"); b = ppg3.FetchJob(outputs="z", url="w")\n'
    out, unresolved = _patch(tmp_path, src, 1)
...
def test_patch_multiline_call_starting_at_recorded_line(tmp_path):
    ...
    assert kwargs == {"outputs": "x", "url": "y", "blake3": _DIGEST}
...
def test_patch_tabs_and_comments_untouched_elsewhere(tmp_path):
    ...
    src = (
        ...
        "\tpass\n"
        "\n"
        'job = ppg3.FetchJob(outputs="x", url="y")  # trailing comment\n'
    )
    ...
    for i, (before, after) in enumerate(zip(before_lines, after_lines)):
        if i == 6:  # 0-indexed line 6 == source line 7, the patched line
            assert after != before
            assert after.startswith(f'job = ppg3.FetchJob(outputs="x", url="y", blake3="{_DIGEST}"')
...
def test_run_tofu_pass_no_unpinned_jobs_is_a_no_op(graph, capsys):
    ppg3.FetchJob(outputs="a", url="http://example.invalid/a", blake3="cd" * 32)
...
def test_run_tofu_pass_libcst_missing_prints_table(graph, tmp_path, capsys, monkeypatch):
    job = ppg3.FetchJob(outputs="inputs/x.bin", url="http://example.invalid/x")
    assert job.blake3 is None
    job._call_site = (str(tmp_path / "unused.py"), 1)
    report = {"job_entries": {job.id: ("ik-x", "oh-x")}}
...
def test_run_tofu_pass_two_jobs_same_site_prints_table(graph, tmp_path, capsys):
    ...
    job_a = ppg3.FetchJob(outputs="a.bin", url="http://example.invalid/a")
    job_b = ppg3.FetchJob(outputs="b.bin", url="http://example.invalid/b")
    job_a._call_site = site
    job_b._call_site = site
    report = {"job_entries": {job_a.id: ("ik-a", "oh-a"), job_b.id: ("ik-b", "oh-b")}}
    core = _FakeCore(
        {
...
def test_run_tofu_pass_patches_real_file(graph, tmp_path, capsys):
    ...
    script.write_text('job = ppg3.FetchJob(outputs="a.bin", url="http://example.invalid/a")\n')
    job = ppg3.FetchJob(outputs="a.bin", url="http://example.invalid/a")
    job._call_site = (str(script), 1)
    report = {"job_entries": {job.id: ("ik-a", "oh-a")}}
...
def test_call_site_recorded_at_definition(graph):
    ...
    job = ppg3.FetchJob(outputs="a", url="http://example.invalid/a")
...
def test_call_site_shared_across_loop_iterations(graph):
    ...
    for i in range(3):
        jobs.append(
            ppg3.FetchJob(outputs=f"a{i}", url=f"http://example.invalid/{i}")
        )
...
def test_call_site_distinct_for_distinct_lines(graph):
    job_a = ppg3.FetchJob(outputs="a", url="http://example.invalid/a")
    job_b = ppg3.FetchJob(outputs="b", url="http://example.invalid/b")
...
def test_call_site_recorded_even_when_blake3_given(graph):
    job = ppg3.FetchJob(
        outputs="a", url="http://example.invalid/a", blake3="ab" * 32
    )
...
def _write_pipeline_script(path, store_dir, project_dir, url):
    ...
    text = (
        ...
        "frozen=False)\n"
        f"job = ppg3.FetchJob(outputs='inputs/data.bin', url={url!r})\n"
...
def test_e2e_tofu_frozen_mode_still_rejects_blake3_none(tmp_path):
    ...
    with pytest.raises(DefinitionError):
        ppg3.FetchJob(outputs="inputs/x.bin", url="http://example.invalid/x")
